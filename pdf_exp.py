...
try:
    ...
    ip1 = PDF_generator(sstalib,'IPT',samples,sample_dist)
    plt.subplot2grid((rows,cols),(0,0))
    plt.title("plot for ip1, mean=%s ,std=%s"%((sstalib['IPT'])['mu'],(sstalib['IPT'])['sigma']))
    plt.xlabel('Delay(ns)')
    plt.ylabel('Probability')
    sns.scatterplot(ip1.delay, ip1.pdf, color='black')
    

    ip2 = PDF_generator(sstalib,'IPT',samples,sample_dist)
    plt.subplot2grid((rows,cols),(0,1))
    plt.title("plot for ip2, mean=%s ,std=%s"%((sstalib['IPT'])['mu'],(sstalib['IPT'])['sigma']))
    plt.xlabel('Delay(ns)')
    plt.ylabel('Probability')
    sns.scatterplot(ip2.delay, ip2.pdf, color='black')

    g1 = PDF_generator(sstalib,gate_type,samples,sample_dist)
    ...
    plt.xlabel('Delay(ns)')
    plt.ylabel('Probability')
    sns.scatterplot(g1.delay, g1.pdf, color='black')
    ...
    plt.xlabel('Delay(ns)')
    plt.ylabel('Probability')
    sns.scatterplot(som.delay, som.pdf, color='black', label="Sum of Max")
    ...
    # # plt.xlabel('Delay(ns)')
    # # plt.ylabel('Probability')
    # sns.scatterplot(mos.delay, mos.pdf, color='red', label="Max of Sum")
